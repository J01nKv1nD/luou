...
static void parse_address(lua_State *l, const char *addr_str, ENetAddress *address) {
	...
	while (*c != 0) {
		...
		if (scanning_port) {
			...
			if (*c == ':') scanning_port = 1;
			else host_str[host_i++] = *c;
		}
		c++;
	}
	host_str[host_i] = '\0';
	...
	address->port = (strcmp("*", port_str) == 0) ? ENET_PORT_ANY : atoi(port_str);
}

static size_t find_peer_index(lua_State *l, ENetHost *enet_host, ENetPeer *peer) {
	for (size_t i = 0; i < enet_host->peerCount; i++) {
		if (peer == &(enet_host->peers[i])) return i;
	}
...
static uintptr_t compute_peer_key(lua_State *L, ENetPeer *peer) {
	const size_t minalign = sizeof(void*) == 8
		? std::min(alignof(ENetPeer), alignof(std::max_align_t))
		: 1;
	uintptr_t key = (uintptr_t)peer;

	if ((key & (minalign - 1)) != 0)
		luaL_error(L, "Cannot push enet peer to Lua: unexpected alignment (pointer is %p but alignment should be %d)", peer, minalign);
...
static void push_peer_key(lua_State *L, uintptr_t key) {
	...
	else if (key > 0x20000000000000ULL)
...
static ENetPacket *read_packet(lua_State *l, int idx, enet_uint8 *channel_id) {
	...
	*channel_id = 0;
	...
	if (argc >= idx + 2 && !lua_isnil(l, idx + 2)) {
		...
		if (strcmp("unsequenced", flag_str) == 0)       flags = ENET_PACKET_FLAG_UNSEQUENCED;
		else if (strcmp("reliable", flag_str) == 0)     flags = ENET_PACKET_FLAG_RELIABLE;
		else if (strcmp("unreliable", flag_str) == 0)   flags = 0;
		else luaL_error(l, "Unknown packet flag: %s", flag_str);
	}
	...
	if (packet == NULL) luaL_error(l, "Failed to create packet");
	return packet;
}
...
static int host_create(lua_State *l) {
	...
	switch (lua_gettop(l)) {
		case 5: if (!lua_isnil(l, 5)) out_bandwidth = (int)luaL_checknumber(l, 5);
		case 4: if (!lua_isnil(l, 4)) in_bandwidth  = (int)luaL_checknumber(l, 4);
		case 3: if (!lua_isnil(l, 3)) channel_count = (int)luaL_checknumber(l, 3);
		case 2: if (!lua_isnil(l, 2)) peer_count    = (int)luaL_checknumber(l, 2);
	}
...
static int host_service(lua_State *l) {
	...
	if (!host) { luaL_error(l, "Tried to index a nil host!"); return 0; }
...
static int host_compress_with_range_coder(lua_State *l) {
	ENetHost *host = check_host(l, 1);
	if (!host) { luaL_error(l, "Tried to index a nil host!"); return 0; }
	lua_pushboolean(l, enet_host_compress_with_range_coder(host) == 0);
	return 1;
}
...
static int host_connect(lua_State *l) {
	...
	switch (lua_gettop(l)) {
		case 4: if (!lua_isnil(l, 4)) data          = (int)luaL_checknumber(l, 4);
		case 3: if (!lua_isnil(l, 3)) channel_count = (int)luaL_checknumber(l, 3);
	}

	ENetPeer *peer = enet_host_connect(host, &address, channel_count, data);
	if (peer == NULL) { luaL_error(l, "Failed to create peer"); return 0; }
...
static int host_channel_limit(lua_State *l) {
	ENetHost *host = check_host(l, 1);
	if (!host) { luaL_error(l, "Tried to index a nil host!"); return 0; }
	enet_host_channel_limit(host, (int)luaL_checknumber(l, 2));
	return 0;
}
...
static int host_bandwidth_limit(lua_State *l) {
	...
	enet_uint32 out_bandwidth = (int)luaL_checknumber(l, 3);
...
static int host_get_peer(lua_State *l) {
	...
	push_peer(l, &(host->peers[peer_index]));
	return 1;
}
...
static int peer_round_trip_time(lua_State *l) {
	ENetPeer *peer = check_peer(l, 1);
	if (lua_gettop(l) > 1) peer->roundTripTime = (int)luaL_checknumber(l, 2);
...
static int peer_last_round_trip_time(lua_State *l) {
	ENetPeer *peer = check_peer(l, 1);
	if (lua_gettop(l) > 1) peer->lastRoundTripTime = (int)luaL_checknumber(l, 2);
...
static int peer_ping_interval(lua_State *l) {
	ENetPeer *peer = check_peer(l, 1);
	if (lua_gettop(l) > 1) enet_peer_ping_interval(peer, (int)luaL_checknumber(l, 2));
...
static int peer_timeout(lua_State *l) {
	ENetPeer *peer = check_peer(l, 1);
	enet_uint32 limit = 0, minimum = 0, maximum = 0;

	switch (lua_gettop(l)) {
		case 4: if (!lua_isnil(l, 4)) maximum = (int)luaL_checknumber(l, 4);
		case 3: if (!lua_isnil(l, 3)) minimum = (int)luaL_checknumber(l, 3);
		case 2: if (!lua_isnil(l, 2)) limit   = (int)luaL_checknumber(l, 2);
	}
...
static int peer_disconnect(lua_State *l) {
	ENetPeer *peer = check_peer(l, 1);
	enet_peer_disconnect(peer, lua_gettop(l) > 1 ? (int)luaL_checknumber(l, 2) : 0);
	return 0;
}

static int peer_disconnect_now(lua_State *l) {
	ENetPeer *peer = check_peer(l, 1);
	enet_peer_disconnect_now(peer, lua_gettop(l) > 1 ? (int)luaL_checknumber(l, 2) : 0);
	return 0;
}

static int peer_disconnect_later(lua_State *l) {
	ENetPeer *peer = check_peer(l, 1);
	enet_peer_disconnect_later(peer, lua_gettop(l) > 1 ? (int)luaL_checknumber(l, 2) : 0);
	return 0;
}

static int peer_index(lua_State *l) {
	ENetPeer *peer = check_peer(l, 1);
	lua_pushinteger(l, find_peer_index(l, peer->host, peer) + 1);
	return 1;
}
...
static int peer_state(lua_State *l) {
	ENetPeer *peer = check_peer(l, 1);
	switch (peer->state) {
		case ENET_PEER_STATE_DISCONNECTED:           lua_pushstring(l, "disconnected");            break;
		case ENET_PEER_STATE_CONNECTING:             lua_pushstring(l, "connecting");               break;
		case ENET_PEER_STATE_ACKNOWLEDGING_CONNECT:  lua_pushstring(l, "acknowledging_connect");    break;
		case ENET_PEER_STATE_CONNECTION_PENDING:     lua_pushstring(l, "connection_pending");       break;
		case ENET_PEER_STATE_CONNECTION_SUCCEEDED:   lua_pushstring(l, "connection_succeeded");     break;
		case ENET_PEER_STATE_CONNECTED:              lua_pushstring(l, "connected");                break;
		case ENET_PEER_STATE_DISCONNECT_LATER:       lua_pushstring(l, "disconnect_later");         break;
		case ENET_PEER_STATE_DISCONNECTING:          lua_pushstring(l, "disconnecting");            break;
		case ENET_PEER_STATE_ACKNOWLEDGING_DISCONNECT: lua_pushstring(l, "acknowledging_disconnect"); break;
		case ENET_PEER_STATE_ZOMBIE:                 lua_pushstring(l, "zombie");                   break;
...
static int peer_receive(lua_State *l) {
	...
	if (lua_gettop(l) > 1) channel_id = (int)luaL_checknumber(l, 2);
...
static int peer_send(lua_State *l) {
	...
	if (ret < 0) enet_packet_destroy(packet);
...
int luaopen_enet(lua_State *l) {
	...
	lua_pushcfunction(l, host_gc, "host_gc");
	...
	lua_setfield(l, -2, "__index");
	lua_pushcfunction(l, peer_tostring, "peer_tostring");
